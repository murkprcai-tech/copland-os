...
# zaehlt aufrufe pro tag in <cache>/copland-rat-usage.json (fuers panel; cache = %LOCALAPPDATA% bzw. ~/.cache/copland)
...
# plattform: windows haelt keys im user-scope (setx), macos/linux im profil (export) -> beides lesen
function Get-UserEnv([string]$n) {
    $v = $null
    if ($env:OS -eq 'Windows_NT') { $v = [Environment]::GetEnvironmentVariable($n, 'User') }
...
$conf = @{
    nemotron = @{
        ...
        key    = Get-UserEnv 'OPENROUTER_API_KEY'
    }
    groq = @{
        url    = 'https://api.groq.com/openai/v1/chat/completions'
        models = @('llama-3.3-70b-versatile')
        key    = Get-UserEnv 'GROQ_API_KEY'
    }
    qwen = @{
    ...
    gemini = @{
        ...
        key    = Get-UserEnv 'GEMINI_API_KEY'
    }
    mistral = @{
        url    = 'https://api.mistral.ai/v1/chat/completions'
        models = @('mistral-large-latest', 'mistral-small-latest')
        key    = Get-UserEnv 'MISTRAL_API_KEY'
    }
}
$c = $conf[$Ki]
...
$uf = Join-Path $env:LOCALAPPDATA 'copland-rat-usage.json'
